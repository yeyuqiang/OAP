...
import com.intel.oap.common.storage.stream.{ChunkInputStream, ChunkOutputStream, DataStore, PMemManager}
import com.intel.oap.common.unsafe.{PMemBlockPlatform, PersistentMemoryPlatform}
...
private[spark] class DiskStore(
  ...
  private val dataStore = {
    ...
    val pmemChunkSize = conf.getSizeAsBytes("spark.memory.pmem.chunk.size", 4 * 1024 * 1024)
    val pmemMetaStore = conf.get("spark.memory.pmem.metastore", "memkind")
    val pmemStoreType = conf.get("spark.memory.pmem.storetype", "memkind")
    val fallbackDisk = conf.get("spark.memory.pmem.fallbackdisk.path", "/tmp" + executorId)
    ...
    if (isDriver) {
      ...
      properties.setProperty("storetype", pmemStoreType)
      properties.setProperty("fallbackDiskPath", fallbackDisk)
//      properties.setProperty("pmemkv_engine", "cmap")
//      properties.setProperty("pmemkv_path", "/dev/shm/pmemkv-db-" + executorId)
//      properties.setProperty("pmemkv_size", "1073741824")
      val pMemManager = new PMemManager(properties)
      logInfo(s"Init PMem with total size ${pmemTotalSize} chunk size ${pmemChunkSize}," +
        s" meta store ${pmemMetaStore}, store type ${pmemStoreType}")
  ...
  def put(blockId: BlockId)(writeFunc: WritableByteChannel => Unit): Unit = {
    ...
    logDebug(s"Attempting to put block $blockId on pmem")
    ...
    logDebug("Block %s stored on pmem in %d ms".format(
      blockId,
  ...
  def getBytes(blockId: BlockId): BlockData = {
    logDebug(s"Attempting to get block $blockId on pmem")
    ...
    securityManager.getIOEncryptionKey() match {
      ...
      case _ =>
        new DiskBlockData(minMemoryMapBytes, maxMemoryMapBytes, blockId, blockSize, dataStore)
    }
  }
  ...
  private def openForWrite(blockId: BlockId): WritableByteChannel = {
    val out = new ChunkOutputStream(blockId.name.getBytes(), dataStore)
...
private class DiskBlockData(
    ...
    blockId: BlockId,
    blockSize: Long,
    dataStore: DataStore) extends BlockData {

  override def toInputStream(): InputStream =
    new ChunkInputStream(blockId.name.getBytes(), dataStore)
  ...
  */
  ...
  override def toNetty(): AnyRef = None
  ...
  private def open() = new ChunkInputStream(blockId.name.getBytes(), dataStore)
}
